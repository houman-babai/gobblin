...
public abstract class KafkaSource<S, D> extends EventBasedSource<S, D> {
    ...
    public List<WorkUnit> getWorkunits(SourceState state) {
        ...
        KafkaWorkUnitPacker packer = KafkaWorkUnitPacker.getInstance(this, state);
        List<WorkUnit> packedUnits = packer.pack(workUnits, numOfMultiWorkunits);
        LOG.warn("**************THERE ARE N WORK UNITS " + packedUnits.size());
    ...
    private WorkUnit getWorkUnitForTopicPartition(KafkaPartition partition, Offsets offsets,
            Optional<State> topicSpecificState) {
        LOG.warn("***************Creating work unit " + offsets.getLatestOffset());
        ...
        LOG.warn(String.format("Created workunit for partition %s: lowWatermark=%d, highWatermark=%d, range=%d", partition,
